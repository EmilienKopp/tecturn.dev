...
test('a team member can stream the rehearsal recording', function () {
    ...
    $this->actingAs($owner)->get(route('rehearsals.audio', ['rehearsal' => $run->id]))->assertSuccessful();
});
...
test('an assigned reviewer can stream the recording', function () {
    ...
    $this->actingAs($reviewer)->get(route('rehearsals.audio', ['rehearsal' => $run->id]))->assertSuccessful();
});
...
test('a stranger cannot stream the recording', function () {
    ...
    $this->actingAs($stranger)->get(route('rehearsals.audio', ['rehearsal' => $run->id]))->assertForbidden();
});
...
test('a run without a recording returns 404', function () {
    ...
    $this->actingAs($user)->get(route('rehearsals.audio', ['rehearsal' => $run->id]))->assertNotFound();
});
